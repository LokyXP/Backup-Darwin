...
class classConfig():
	...
	def CheckConfig(self):
		...
		if self.Config.read(f'{os.path.dirname(os.path.realpath(__file__))}/config.ini') != []:
			...
			self.Config = ConfigParser()
			self.Config.write('vision_yolov7/vision_yolov7/config.ini')
...
#continua aqui, tentando estabilizar o centralizando a bola
			...
			with open('src/vision_yolov7/vision_yolov7/config.ini', 'wb') as configfile:
