...
class BallDetection(Node):
    ...
    def main_callbalck(self):
        ...
        if(ret):
            if(self.get_image and (time.time() - self.old_time > 0.5)):
                self.get_logger().info("Perdeu a bola")
                self.old_time = time.time()
    ...
    def predict_image(self, img):
        results = self.model(img, device=self.device, conf=0.7, max_det=3, verbose=False, imgsz=[320, 448])
        return results[0]
    ...
    def find_ball(self):
        ...
        if ball_detection.size > 0:
            ...
            #self.get_logger().info("%f", ball_pos.x)
            #self.get_logger().info('Posisao x "%f"' % self.ball_pos.x)
            #self.get_logger().info('Posisao y "%f"' % self.ball_pos.y)
