...
class DecisionNode(Node):
    ...
    def send_goal(self, order):
        ...
        if order != self.last_movement: # Se tiver que mudar o movimento
            if (order == 16 or order == 17 or order == 18 or order == 19 or self.last_movement == 8): # Robô caído: prioridade é levantar, por isso ele cancela o que estiver fazendo
            ...
            if(ROBOT_NUMBER != 1): # JOGADOR
                if(self.gamestate == 0): # Initial state - Robô parado em pé
                    ...
                    if (self.contador < 3000):
                        self.walking()
                        self.contador+=1
                    elif (self.gyro_z > 1.57 and self.gyro_z < -1.57): 
                        self.turn_left()
                    ...
                    if(self.secstate == 6): # Penalti do oponente
                        ...
                        if(self.BALL_DETECTED == False):
                            ...
                            if (self.save_ball_left and self.cont_turn < 250):
                                self.turn_left()
                                self.cont_turn += 1
                            elif (self.save_ball_right and self.cont_turn < 250):
                                self.turn_right()
                                self.cont_turn += 1
