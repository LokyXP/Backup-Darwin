  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = custom_interfaces__srv__Reset_Request;
    is_plain =
      (
      offsetof(DataType, set_mag_ref) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _Reset_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_custom_interfaces__srv__Reset_Request(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = custom_interfaces__srv__Reset_Response;
    is_plain =
      (
...
static size_t _Reset_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_custom_interfaces__srv__Reset_Response(
    full_bounded, is_plain, 0);
