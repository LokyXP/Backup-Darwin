...
bool custom_interfaces__msg__joint_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("custom_interfaces.msg._joint_state.JointState", full_classname_dest, 45) == 0);
  }
  custom_interfaces__msg__JointState * ros_message = _ros_message;
  {  // id
  ...
  {  // info
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int32_t * dest = ros_message->info.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * custom_interfaces__msg__joint_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("custom_interfaces.msg._joint_state");
  ...
  {  // info
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "info");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->info.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int32_t * src = &(ros_message->info.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->info.size * sizeof(int32_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
