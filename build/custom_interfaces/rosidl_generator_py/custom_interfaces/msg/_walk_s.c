...
bool custom_interfaces__msg__walk__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // walk_number
    PyObject * field = PyObject_GetAttrString(_pymsg, "walk_number");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->walk_number = (uint8_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
...
PyObject * custom_interfaces__msg__walk__convert_to_py(void * raw_ros_message)
{
  ...
  {  // walk_number
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->walk_number);
    {
      int rc = PyObject_SetAttrString(_pymessage, "walk_number", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
